...
function getServices(config: ProjectConfig, nodes: CanvasNode[]): ServiceDescriptor[] {
  ...
  return source.map((node, index) => {
    const name = String(node.config.serviceName || `Service ${index + 1}`);
...
${config.description || 'A project template generated by ServiceSmith from an architecture topology.'}

## Technology stack

- ${config.stack === 'spring' ? `Java ${config.javaVersion} / Spring Boot ${config.springBootVersion}` : `Go ${config.goVersion} / Iris v12`}
- Layer structure: Controller → Service → DAO
- Service modules: ${services.map((service) => service.moduleName).join(', ')}
- Topology capabilities: ${Object.entries(features).filter(([, enabled]) => enabled).map(([name]) => name).join(', ') || 'base service'}
...
When container support is enabled, run:
